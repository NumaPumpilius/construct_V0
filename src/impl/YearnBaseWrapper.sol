...
interface StrategyAPI {
    ...
    function keepCRV() external view returns (uint256);
 ...
 *
 */
abstract contract YearnBaseWrapper is Initializable {
        ...
        for (uint256 id = 0; id < vaults.length; id++) {
            ...
            if (availableShares > 0) {
                ...
                if (amount != WITHDRAW_EVERYTHING) {
                    ...
                    uint256 estimatedShares = amount.sub(withdrawn).mul(10**uint256(vaults[id].decimals())).div( // NOTE: Changes every iteration
                        vaults[id].pricePerShare()
                    ); // NOTE: Every Vault is different
